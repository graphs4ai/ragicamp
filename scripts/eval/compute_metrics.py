...
def compute_per_question_metrics(
    predictions_data: Dict[str, Any], metrics: List[Any]
) -> List[Dict[str, Any]]:
...
def compute_metric_statistics(
    ...
    for item in per_question_metrics:
        for key in item.keys():
            if key not in ["question_index", "question_id", "question"] and isinstance(
...
Examples:
  ...
  python scripts/compute_metrics.py --predictions outputs/agent_predictions_raw.json --config configs/my_config.yaml --resume
    ...
    parser.add_argument(
        "--config", type=str, help="Path to experiment config (for metric and judge model settings)"
    )
    ...
    parser.add_argument(
        "--output", type=str, help="Output directory (default: same as predictions file)"
    )
    ...
    overall_metrics = compute_metrics_on_predictions(
        predictions_data, metrics, judge_model, args.checkpoint_dir
    )
    ...
    # Save results
    save_results_with_metrics(predictions_data, overall_metrics, per_question_metrics, output_dir)
