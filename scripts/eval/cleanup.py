...
def cleanup_outputs(base_path: Path, delete: bool = False, keep_latest: int = 0) -> dict:
    ...
    if invalid_runs:
        ...
        for run_dir, reason in invalid_runs:
            ...
            rel_path = (
                run_dir.relative_to(base_path) if run_dir.is_relative_to(base_path) else run_dir
            )
            print(f"  ❌ {rel_path}")
...
Examples:
    ...
    parser.add_argument(
        "--delete", "-d", action="store_true", help="Actually delete failed runs (default: dry run)"
    )
