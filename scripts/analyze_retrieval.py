...
def main():
    ...
    # Output
    if args.csv:
        print("experiment,total,recall,gen_given_ctx,bottleneck,correct_w_ctx,wrong_w_ctx,correct_wo_ctx,wrong_wo_ctx")
        for s in all_stats:
            print(f"{s.name},{s.total},{s.retrieval_recall:.3f},{s.generation_given_context:.3f},{s.bottleneck},"
        ...
        for s in all_stats:
            print(f"{s.name[:60]:<60} {s.total:>6} {s.retrieval_recall:>7.1%} {s.generation_given_context:>7.1%} {s.bottleneck:>12}")
        ...
        avg_gen = sum(s.correct_with_context for s in all_stats) / total_with_ctx if total_with_ctx else 0
