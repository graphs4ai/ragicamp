...
def main():
    ...
    parser.add_argument(
        "--checkpoint-every", type=int, default=1, help="Checkpoint every N batches"
    )
    parser.add_argument(
        "--index-type",
        choices=["hnsw", "flat"],
    ...
    if args.start_batch > 0 and index_path.exists():
        ...
        if args.index_type == "hnsw":
            ...
            print(
                "  (Flat uses less memory but slower search - can convert to HNSW later)",
                flush=True,
            )
    ...
    print(
        f"✓ Embeddings processed: {total_added:,} vectors in {time.time() - t_start:.1f}s",
        flush=True,
    )
        ...
        while True:
            try:
                ...
                if chunk_batch_num % 10 == 0:
                    print(
                        f"  Loaded {chunk_batch_num} batches, {len(all_chunks):,} chunks...",
                        flush=True,
                    )
            except EOFError:
                break
    ...
    if total_added != len(all_chunks):
        print(
            f"WARNING: Count mismatch! {total_added} embeddings vs {len(all_chunks)} chunks",
            flush=True,
        )
        print("  This may indicate corrupted data.", flush=True)
