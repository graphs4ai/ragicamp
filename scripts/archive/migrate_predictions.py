...
def main():
    ...
    if is_experiment:
        ...
        migrated = [
            (n, r) for n, r in results.items() if r.get("status") in ("migrated", "would_migrate")
        ]
        if migrated:
            print("\nMigrated experiments:")
            for name, result in migrated:
                print(
                    f"  - {name}: {result.get('predictions_migrated', 0)}/{result.get('total_predictions', 0)} predictions"
                )
