...
def check_prerequisites(retrievers: List[str]) -> bool:
    ...
    if issues:
        ...
        print(
            "  python scripts/data/index.py --embedding-model all-mpnet-base-v2 --name wikipedia_small_chunked_mpnet"
        )
        return False
...
Examples:
    ...
    if not args.dry_run:
        ...
        summary_path = (
            output_dir / f"rag_baseline_study_summary_{start_time.strftime('%Y%m%d_%H%M%S')}.json"
        )
