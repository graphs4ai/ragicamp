...
from ragicamp.core import EvaluationError, get_logger
...
def build_legacy_config(cfg: DictConfig) -> Dict[str, Any]:
    ...
    legacy = {
        ...
        "evaluation": {
            k: v for k, v in config.get("evaluation", {}).items() if k != "dataset_override"
        },
...
def main(cfg: DictConfig) -> Optional[Dict[str, Any]]:
    ...
    print(
        f"Dataset: {cfg.dataset.get('name', 'unknown')} ({cfg.dataset.get('num_examples', 'all')} examples)"
    )
